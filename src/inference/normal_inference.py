...
class PrefetchDataset(torch.utils.data.Dataset):
    def __init__(self, opt, dataset, pre_process_func):
        ...
        for v in self.vlist:
            ...
            if self.opt.pa_model != '':
                for i in range(min(self.opt.K * self.opt.ninput, self.nframes[v])  - self.opt.ninput + 1 , 1 + self.nframes[v]): # start: self.opt.K ||+ (self.opt.ninput - 1)
                    ...
                    # remove this check as i don't want intermediate results cached there (possibly distort accuracy)
                    #if not os.path.exists(self.outfile(v, i)):
                    self.indices += [(v, i)]
        ...
        # to sparsely debug test set
        #self.indices = self.indices[::100]
        '''
        
        # MOD: take into mem + reverse + only long-range memory
        for v in self.vlist:
            for i in reversed(range(self.nframes[v]//2 , 1 + self.nframes[v])):
                if not os.path.exists(self.outfile(v, i)):
                    self.indices += [(v, i)]
        '''
    ...
    def __getitem__(self, index):
        ...
        if self.opt.rgb_model != '':
            ...
            for _ in range(1, n_mem+1):
                cur_f = np.maximum(cur_f - self.opt.ninputrgb, 1) # +1: motion frames and rgb frame overlap by 1
        ...
        if self.opt.flow_model != '':
            ...
            for idx, i in enumerate(im_inds): 
                for ii in range(self.opt.ninput):
                    flows.append(cv2.imread(self.flowfile(v, max(i + 1 - ii, 1))).astype(np.float32))
                    im_inds_flow.append(max(i - ii, 0))
