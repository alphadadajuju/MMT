    ...
    for iv, v in enumerate(vlist):
        ...
        for ilabel in range(len(dataset.labels)):
            ...
            for frame in range(min(K  * opt.ninput , dataset._nframes[v]) - opt.ninput + 1  , 1 + dataset._nframes[v]):
                ...
                if frame == first_frame: # orig: 1 
                    for i in range(ltubelets.shape[0]):
                        CURRENT_TUBES.append([(first_frame, ltubelets[i, :])]) # orig: 1
                    continue
                ...
                for it, t in enumerate(CURRENT_TUBES):
                    ...
                    last_frame, last_tubelet = t[-1] # confusing -1? -> # a tube could contain multiple linked mini-tubes (linked over time)
                    ious = []
            ...
            for t in FINISHED_TUBES:
                ...
                beginframe = t[0][0] # TODO: needs to be taken care of (forward vs backward)
                endframe = t[-1][0] + K - 1 # TODO
