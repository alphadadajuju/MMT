...
def tubelet_in_tube(tube, i, K):
    ...
    # ORIG: very difficult to debug
    # NOTE: incorrect checking in modified MOC! Only worked in ascending frame order 
    ...
    for j in range(lo_, hi_+1):
        is_f_in_tube.append(j in tube[:,0])
    
    return all(is_f_in_tube)
...
def tubelet_out_tube(tube, i, K):
    ...
    hi_ = max(i+K+1, i)
    for j in range(lo_, hi_+1):
        is_f_out_tube.append(not j in tube[:,0])
    
    return all(is_f_out_tube)
...
def nms_tubelets(dets, overlapThresh=0.3, top_k=None):
    ...
    counter = 0
    '''
    # added: to DEBUG by visualizing what the hell is actually going on?
    for k in range(1):
        for bb in range(len(order)):
            plt.plot([x1[k][bb], x2[k][bb]], [y1[k][bb], y1[k][bb]], 'b-')
            plt.plot([x1[k][bb], x1[k][bb]], [y1[k][bb], y2[k][bb]], 'b-')
            plt.plot([x1[k][bb], x2[k][bb]], [y2[k][bb], y2[k][bb]], 'b-')
            plt.plot([x2[k][bb], x2[k][bb]], [y1[k][bb], y2[k][bb]], 'b-')
        
        plt.xlim([0,320])
        plt.ylim([0,240])
        plt.show()
    '''
    # initial experiment shows that this below snippet does nothing?
    while order.size > 0:
        i = order[0]
        counter += 1
    ...
    '''
    # added: to DEBUG by visualizing what the hell is actually going on?
    # Coordinates of bounding boxes AFTER nms
    x1 = [dets[:, 4 * k] for k in range(K)]
    y1 = [dets[:, 4 * k + 1] for k in range(K)]
    x2 = [dets[:, 4 * k + 2] for k in range(K)]
    y2 = [dets[:, 4 * k + 3] for k in range(K)]
